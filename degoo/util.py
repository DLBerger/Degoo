...
def mkpath(path, verbose=0):
    ...
    if verbose>1: print(f"{CWD=} {path=} {verbose=} {dirs=}")
    ...
    if verbose>1: print(f"{dirs=} {current_dir=}")
    for d in dirs:
...
def mkdir(name, parent_id=None, verbose=0, dry_run=False):
    ...
    '''
    if verbose>1: print(f"{verbose=} {parent_id=} {CWD=}")
    if parent_id is None:
        if "Path" in CWD:
            parent_id = get_item(CWD["Path"]).get("ID", None)
        else:
            parent_id = 0
        if verbose>1: print(f"New {parent_id=}")
    ...
    if name in existing_names:
        ...
        if not dry_run:
            ID = api.setUploadFile3(name, parent_id, verbose)
        else:
...
def put_file(local_file, remote_folder, verbose=0, if_changed=False, dry_run=False, schedule=False):
    ...
    if (not if_changed or has_changed(local_file, remote_folder, verbose - 1)):
        if dry_run:
            ...
            # Provides the metadata we need for the upload
            ...
            # last an apparent filename that is constructed as checksum.extension.
            ...
            Fields = {
                ...
                'file': (filename, open(local_file, 'rb'), MimeTypeOfFile)
            }
            ...
            if response.ok and response.status_code == 204:
                # There's a Google Upload ID returned in the headers. Not sure what use it is.
...
#                             "&use-cf-cache=true"]) # @UnusedVariable
                ...
                if verbose>1:
                    print(f"api.setUploadFile3({filename=}, {dir_id=}, {verbose=}, {Size=}, {Checksum=})")
                ...
                if verbose>1:
                    print(f"api.getOverlay3({degoo_id=})")
...
#                     URL = expected_URL
                ...
                if verbose>1:
                    print(f"returning {degoo_id=}, {Path=}, {URL=}")
        ...
        if verbose>1:
            print(f"About to call get_children({dir_id})")
        children = get_children(dir_id)
...
def put_directory(local_directory, remote_folder, verbose=0, if_changed=False, dry_run=False, schedule=False):
    ...
    if verbose>1:
        print(f"{local_directory=} {remote_folder=}")
    ...
    if verbose>0:
        if dry_run:
            print(f"Would upload directory {local_directory} to {target_dir['Path']}")
        else:
            print(f"Uploading directory {local_directory} to {target_dir['Path']}")
    if verbose>1:
        print(f"{target_dir=}, {target_junk=}, {target_name=}")
    ...
    for root, dirs, files in os.walk(local_directory):
        ...
        for name in dirs:
            if verbose>1:
                print(f"Dir: {name=}, {target_junk=}, {relative_root=}({IDs[relative_root]})")
            Name = os.path.join(relative_root, name)
            
            if verbose>0:
                if dry_run:
                    print(f"Would create directory {target_dir['Path']+os.sep}{Name}")
                else:
                    print(f"Creating directory {target_dir['Path']+os.sep}{Name}")
            IDs[Name] = mkdir(name, IDs[relative_root], verbose - 1, dry_run)

        for name in files:
            Name = os.path.join(relative_root, name)
            if target_junk:
                Name = os.path.join(target_junk, Name)

            if verbose>1:
                print(f"File: {name=}, {target_junk=}, {relative_root=}({IDs[relative_root]}) {Name=} {verbose=} {if_changed=}")
